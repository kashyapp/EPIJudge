...
class Solution {
    ...
    fun getSkyline(buildings: Array<IntArray>): List<IntArray> {
        val points = arrayListOf<Solution.Point>()
        for (i in buildings.indices) {
            val b = buildings[i]
            points.add(Solution.Point(b[0], b[2], i, true))
            points.add(Solution.Point(b[1], b[2], i, false))
        }
    ...
    private fun addKeyPoint(keyPoints: ArrayList<IntArray>, x: Int, y: Int) {
        ...
        if (x == keyPoints.last()[0]) {
            keyPoints.removeAt(keyPoints.size - 1)
            addKeyPoint(keyPoints, x, y)
        } else if (y != keyPoints.last()[1]) {
            keyPoints.add(intArrayOf(x, y))
        }
    }
}
